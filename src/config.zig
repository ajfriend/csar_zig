...
//! API — user-tunable parameters live in `SolveOptions` in `csar.zig`.
...
pub const algo = struct {
    ...
    /// with the point count and overruns `max_outer`.
    ///
    ...
    /// farthest-point; `farthestPointSeed` in `csar.zig`) so FW *grows* into the
