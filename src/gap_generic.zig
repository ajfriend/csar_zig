...
//! f64 in csar.zig.
//!
//! T-typed constants: `sigma0` and the KW `budget` are derived AT `T`
//! (a comptime f64 constant would silently type the computation and
//! round f128 work through f64 — the hazard recorded on #94). The
//! comptime guard below pins the f64 instantiation to the config
//! literal, per the "reproduces the literal in force" rule.
//!
...
pub fn Gap(comptime T: type) type {
    return struct {
        ...
        /// The structural axial eigenvalue. The expression stays in
        /// comptime_float and rounds ONCE on coercion to `T`: at f64
        /// that reproduces `config.SIGMA_0` bit-exactly (the comptime
        /// guard pins it), at f128 it is full-precision. An
        /// `@as(T, ...)` spelling would round twice (sqrt, then
        /// divide) and the guard below catches exactly that.
