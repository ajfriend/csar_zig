//! The generic linalg namespace — every float-typed primitive over a
//! comptime scalar `T`. Split from linalg.zig so the f64 aliases there
//! (the public spellings) don't shadow the names inside the namespace.
//! See linalg.zig's header for the conventions; #9 phase 2 extends
//! this pattern to the whole solver.
...
/// is re-exported below under the original names, so existing callers
/// compile the identical code through aliases (no wrappers); the f128
/// instantiation serves the certificate slice's gap oracle.
/// Transcendentals route through qmath (argument-typed, so they
/// follow `T` automatically). `LU` stays f64-only below: it belongs
/// to the Newton polish, outside the slice.
pub fn Linalg(comptime T: type) type {
    ...
    pub const Vec3 = extern struct {
        ...
        pub fn randomUnit(rng: std.Random) Vec3 {
            return (Vec3{ .m = .{
                @as(T, rng.floatNorm(f64)), @as(T, rng.floatNorm(f64)), @as(T, rng.floatNorm(f64)),
            } }).normalize();
        }
    };
    ...
    pub const Vec2 = extern struct {
        ...
        pub inline fn norm(v: Vec2) T { return qmath.sqrt(v.dot(v)); }
        pub inline fn scale(v: Vec2, s: T) Vec2 { return .{ .m = .{ s * v.m[0], s * v.m[1] } }; }
    ...
    pub const Mat3 = struct {
        ...
        pub fn randomNormal(rng: std.Random) Mat3 {
            var r: Mat3 = undefined;
            for (0..9) |i| r.m[i] = @as(T, rng.floatNorm(f64));
            return r;
        }
