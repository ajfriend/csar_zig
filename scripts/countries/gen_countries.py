...
scripts/countries/data/countries.json with, per country:
  - `vertices`: the flat union of every ring's corners as unit 3-vectors
    (the format `csar.solve` consumes — for the aspect ratio the input is just
...
polar cap well inside a hemisphere, so csar solves it fine).
...
def polygon_rings(geom) -> list[list[list[float]]]:
    ...
    for poly in polys:
        for ring in [poly.exterior, *poly.interiors]:
            ...
            # don't feed csar a coincident point (cf. gen_states.py).
