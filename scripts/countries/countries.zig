...
//! `csar.solve` on every country's flattened vertex set, and writes
...
//! `.infeasible` by csar's Farkas/halfspace test, and we report+skip rather
...
// nowhere near the DGGS sub-meter duality-gap floor — so the strict csar
...
// need more than csar's default 100 outer iterations to certify the gap at
...
/// Tally of `csar.solve` outcomes across all countries.
const Counts = struct {
    converged: u32 = 0,
    infeasible: u32 = 0,
...
pub fn main(init: std.process.Init) !void {
    ...
    var counts: Counts = .{};
    const results = try processCountries(init.io, allocator, out_alloc, &counts);
...
fn processCountries(
    ...
    for (input.countries) |country| {
        const outcome_or_err = csar.solve(gpa, country.vertices, .{
...
fn writeOutput(io: std.Io, results: []CountryResult) !void {
