//! Step 2 of the US-states aspect-ratio example.
//!
//! Reads scripts/states/data/states.json (produced by step 1), calls
//! `csar.solve` on every state's flattened vertex set, and writes
...
// sub-meter κ·ε duality-gap floor that forces finest-resolution DGGS cells down
// to 1e-3. The strict csar default converges cleanly here, so keep it.
...
/// Tally of `csar.solve` outcomes across all states.
const Counts = struct {
    converged: u32 = 0,
    infeasible: u32 = 0,
