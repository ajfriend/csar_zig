...
```zig
// Before:
const info = try csar.solve(allocator, X, .{});
defer info.deinit();
switch (info.status) {
...
// After:
var outcome = try csar.solve(allocator, X, .{});
...
Internal:
- `src/csar.zig` — `solve` body: assemble per-variant outcomes
...
- `src/csar.zig` — `checkFeasibility(info, X)` — currently takes
  `Info` and returns `+inf` if `info.status != .converged`. Switch
  to taking `Converged` (or `Outcome` and switching internally).
- `src/csar.zig` — `Info` struct + methods deleted; replaced by the
...
1. Define `Outcome` + per-variant structs in `src/csar.zig`. Keep
   `Info` + `Status` temporarily so the build stays green.
2. Add an `Info → Outcome` adapter or rewrite `solve` directly.
3. Migrate `src/csar.zig`'s `solve` body to construct `Outcome`
   variants directly.
