...
- `probe2.zig` — failing-case trace + centroid-init A/B (uses
  `csar.probe_trace` / `csar.probe_centroid_init`).
- `probe3.zig` — trust-region step-cap sweep (`csar.probe_step_cap`).
- `probe4.zig` — inner-FW budget × step cap grid (`csar.probe_inner_iters`).
