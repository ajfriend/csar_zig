...
hand-rolled arithmetic in `src/csar.zig` and `src/newton.zig` that
...
### B3. `det_G` in `mveeFw` (`src/csar.zig:245`)
...
### B4. Coplanarity discriminant (`src/csar.zig:631`)
...
### C6. `rescaleP` sum-of-squares (`src/csar.zig:74`)
...
### C7. `denom` in `recoverAPerp`-adjacent code (`src/csar.zig:292`)
