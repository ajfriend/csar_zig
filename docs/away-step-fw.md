...
`mveeFwAway` was implemented (kept in-tree in `src/csar.zig` for the
...
2. **Eliminate the near-singular drop-step hazard at the source.** The
   ...
   `src/csar.zig` `mveeFw`) fires on noise-level descent signals at
...
- `src/csar.zig` — `mveeFw` (the loop to replace), `initWeights` /
