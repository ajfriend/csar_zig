...
landed). Scope was the whole solver — `src/csar.zig` and supporting
...
- **Location:** `src/csar.zig` — the L = V·√Λ construction inside
...
- **Location:** `src/csar.zig` — `recoverAPerp`'s `s_det` and `denom`
  computations.
- **Severity:** silent NaN propagation. Critical.
...
```zig
var last_info: ?csar.Info = null;
for (0..N_RUNS) |r| {
    const info = try csar.solve(...);  // may error
...
- **Location:** `src/csar.zig` — `halfspaceCheck`'s opening
...
**Failure scenario.** Caller does `csar.solve(allocator,
...
- **Location:** `src/csar.zig` — three sites:
...
- **Location:** `src/csar.zig` — `Info.b()` and `Info.A()` methods;
...
- **Location:** `src/csar.zig` — `isCoplanarInput`'s `tr <= 0` guard.
...
- **Location:** `src/csar.zig` — the `nz <= NEAR_SING` branch in
...
- **Location:** `src/csar.zig` — the `if (coplanarity_tol > 0 ...)`
...
- **Location:** `src/csar.zig` — `Cert` struct doc-comment for
