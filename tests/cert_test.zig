...
test "repair: gap is invariant under scaling A; off-axis candidates still certify" {
    ...
    const co = try cert.cert_primal(allocator, pts, c.A(), c.b());
    var cd = co.certified;
    defer cd.deinit();
    ...
    // is unchanged.
    const co13 = try cert.cert_primal(allocator, pts, c.A().scale(1.3), c.b());
    ...
    const co_off = try cert.cert_primal(allocator, pts, c.A(), b_off);
...
test "OOM in certify's last alloc hits the indices errdefer" {
    ...
    {
        const co = try cert.cert_primal(count_fa.allocator(), pts, c.A(), c.b());
        var cd = co.certified;
        cd.deinit();
    }
    ...
    try std.testing.expectError(
        error.OutOfMemory,
        cert.cert_primal(fa.allocator(), pts, c.A(), c.b()),
    );
}
...
test "no_certificate: every reason, from both entry points" {
    ...
    try std.testing.expect(cert.cert_dual(&octant, neg, b_ok, &lam1).no_certificate == .not_psd);
    const co_psd = try cert.cert_primal(allocator, &octant, neg, b_ok);
    ...
    try std.testing.expect(cert.cert_dual(&octant, eye, b_zero, &lam1).no_certificate == .axis_not_interior);
    const co_z = try cert.cert_primal(allocator, &octant, eye, b_zero);
    try std.testing.expect(co_z.no_certificate == .axis_not_interior);
    const b_edge = csar.Vec3{ .m = .{ 1, 0, 0 } }; // b·(0,1,0) = 0
    try std.testing.expect(cert.cert_dual(&octant, eye, b_edge, &lam1).no_certificate == .axis_not_interior);
    const co_e = try cert.cert_primal(allocator, &octant, eye, b_edge);
    ...
    try std.testing.expect(cert.cert_dual(&octant, eye, b_ok, &lam0).no_certificate == .empty_support);

    // dual_indefinite: a single-point λ leaves Z rank-deficient.
    const lam_one = [_]f64{ 1, 0, 0 };
    try std.testing.expect(cert.cert_dual(&octant, eye, b_ok, &lam_one).no_certificate == .dual_indefinite);
    // certify pass-through: two points can never pin a full-rank dual.
    const two = [_][3]f64{ .{ 1, 0, 0 }, .{ 0, 1, 0 } };
    const co_two = try cert.cert_primal(allocator, &two, eye, b_ok);
