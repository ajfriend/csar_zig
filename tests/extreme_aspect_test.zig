...
fn randomRotation(rng: std.Random) csar.Mat3 {
    var r = csar.Mat3.randomNormal(rng);
    r.orthonormalize();
    return r;
}
...
test "extreme aspect ratio: three geometries, rotation-invariant" {
    ...
    for (cases) |case| {
        ...
        var canon_outcome = try csar.solve(allocator, canon_pts[0..], .{ .gap_tol = tol, .coplanarity_tol = 1e-12, .max_outer = max_outer });
        ...
        while (k < n_rotations) : (k += 1) {
            ...
            var rot_outcome = try csar.solve(allocator, rot_pts[0..], .{ .gap_tol = tol, .coplanarity_tol = 1e-12, .max_outer = max_outer });
...
test "coplanarity check cutoff is near the parameter's value" {
    ...
    // Above cutoff: ratio ≈ 4.3e-11 (43× above tol). Must not flag.
    {
        ...
        var outcome = try csar.solve(allocator, pts[0..], .{ .gap_tol = tol, .coplanarity_tol = coplanarity_tol, .max_outer = max_outer });
    ...
    // Below cutoff: ratio ≈ 2.7e-14 (37× below tol). Must flag.
    {
        ...
        try std.testing.expectError(
            ...
            csar.solve(allocator, pts[0..], .{ .gap_tol = tol, .coplanarity_tol = coplanarity_tol, .max_outer = max_outer }),
        );
    }
    ...
    {
        var pts: [3][3]f64 = arcPoints(90.0, 5.0e-8);
        var outcome = try csar.solve(allocator, pts[0..], .{ .gap_tol = tol, .coplanarity_tol = 1e-20, .max_outer = max_outer });
        defer outcome.deinit();
    }
}
...
test "coplanarity check flags great-circle inputs" {
    ...
    try std.testing.expectError(
        csar.InputError.CoplanarInput,
        csar.solve(allocator, canon_pts[0..], .{ .gap_tol = tol, .coplanarity_tol = coplanarity_tol, .max_outer = max_outer }),
    );
    ...
    while (k < 10) : (k += 1) {
        ...
        try std.testing.expectError(
            csar.InputError.CoplanarInput,
            csar.solve(allocator, rot_pts[0..], .{ .gap_tol = tol, .coplanarity_tol = coplanarity_tol, .max_outer = max_outer }),
        );
    }
...
test "solve rejects malformed inputs with typed errors" {
    ...
    try std.testing.expectError(
        csar.InputError.InsufficientPoints,
        csar.solve(allocator, &[_][3]f64{}, opts),
    );
    try std.testing.expectError(
        csar.InputError.InsufficientPoints,
        csar.solve(allocator, ok_pts[0..1], opts),
    );
    try std.testing.expectError(
        csar.InputError.InsufficientPoints,
        csar.solve(allocator, ok_pts[0..2], opts),
    );

    // gap_tol: must be finite and positive.
    try std.testing.expectError(
        csar.InputError.InvalidTolerance,
        csar.solve(allocator, ok_pts[0..], .{ .gap_tol = -1.0 }),
    );
    try std.testing.expectError(
        csar.InputError.InvalidTolerance,
        csar.solve(allocator, ok_pts[0..], .{ .gap_tol = 0.0 }),
    );
    try std.testing.expectError(
        csar.InputError.InvalidTolerance,
        csar.solve(allocator, ok_pts[0..], .{ .gap_tol = std.math.nan(f64) }),
    );
    try std.testing.expectError(
        csar.InputError.InvalidTolerance,
        csar.solve(allocator, ok_pts[0..], .{ .gap_tol = std.math.inf(f64) }),
    );
    ...
    try std.testing.expectError(
        csar.InputError.InvalidTolerance,
        csar.solve(allocator, ok_pts[0..], .{ .coplanarity_tol = std.math.nan(f64) }),
    );
}
...
fn runWithFailIndex(fail_index: usize, X: []const [3]f64, opts: csar.SolveOptions) !csar.Outcome {
    var fa = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = fail_index });
    return csar.solve(fa.allocator(), X, opts);
}

fn lastAllocFailIndex(X: []const [3]f64, opts: csar.SolveOptions) !usize {
    var fa = std.testing.FailingAllocator.init(std.testing.allocator, .{});
    var outcome = try csar.solve(fa.allocator(), X, opts);
    outcome.deinit();
...
test "OOM in the last cert alloc hits buildFarkasCert's indices errdefer" {
    ...
    const opts: csar.SolveOptions = .{ .coplanarity_tol = -1 };
...
test "acceptBUpdate fallback: all backtracks fail when a point sits below FEAS_MARGIN" {
    ...
    // setup directly to hit lines 174-176 of src/csar.zig.
    const acceptBUpdate = core.acceptBUpdate;
    const b = csar.Vec3{ .m = .{ 1, 0, 0 } };
    const Q = b.orthoBasis();
    // x dotted with b equals 1e-9 — below FEAS_MARGIN = 1e-8.
    const x_eps: f64 = 1e-9;
    const x = csar.Vec3{ .m = .{ x_eps, @sqrt(1.0 - x_eps * x_eps), 0 } };
    const Xw = [_]csar.Vec3{x};
